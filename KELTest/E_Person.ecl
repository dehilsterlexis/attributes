...
EXPORT E_Person := MODULE
  ...
  EXPORT __Result := __CLEARFLAGS(__PreResult) : PERSIST('~temp::KEL::KELTest::Person::Result',EXPIRE(7));
  ...
  EXPORT Composite := MODULE
    ...
    EXPORT Modules_PersonSet_Big_File_PayloadByUID := INDEX(Modules_PersonSet_Big_File_Dedup,{__UID},{Modules_PersonSet_Big_File_Dedup},'~key::KEL::KELTest::E_Person::Composite::__EE15::Payload');
    ...
    SHARED Build_File := OUTPUT(Stream,,'~key::KEL::KELTest::Person::Composite::File',COMPRESSED,OVERWRITE);
    EXPORT File := DATASET('~key::KEL::KELTest::Person::Composite::File',{Layout,KEL.typ.int __RecPtr{virtual(fileposition)}},FLAT);
    EXPORT File_Index := INDEX(File,{UID,__RecPtr},'~key::KEL::KELTest::Person::Composite::Index');
